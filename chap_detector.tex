...
The \L1 jet trigger uses the transverse energy sums computed in the calorimeter (both hadronic and electromagnetic) trigger regions. Each region consists of $4 \times 4 $  trigger tower windows, spanning a region of $\Delta \eta \times \Delta \phi = 0.087 \times 0.087$ in pseudorapidity-azimuth. The jet trigger uses a $3 \times 3$ calorimeter region (144 trigger towers) sliding window technique within the barrel of the \ac{CMS} calorimeter, as shown in Figure \ref{fig:l1jetalgo}. 

In forming a \L1 jet is it required that the central region to be higher than the eight neighbouring regions $ E_{\text{T central}} \geq E_{\text{T surround}}$. Additionally, a minimum threshold of 5 \GeV on $E_{\text{T central}}$ was introduced during the 2012 run period to suppress noise from pile-up. A comparison between these two configurations is shown in Section (\ref{subsec:l1jetseedpu}).
