...
The \L1 jet efficiency is defined as the fraction of leading offline jets which were matched with a L1 $\tau$- or central jet above a certain trigger threshold, divided by all events in the sample with at least a single offline jet above threshold. 
...
The mean, $\mu$, values are measured to sit at higher \theht for both benchmarked \theht triggers, whilst a better resolution is also observed after the introduction of the jet seed threshold. These values can be found within Table \ref{tab:l1jetseedtableHT}.
...
When comparisons are made between the individual pile-up scenarios, it can be seen that in the presence of higher pile-up , $\mu$ is seen to shift to larger values and a poorer resolution, $\sigma$, observed. These differences between the different pile-up scenarios, can once again be attributed to an increasing number of soft pile-up jets that add to the transverse energy of the lead jet from the primary interaction within each successive category. 
