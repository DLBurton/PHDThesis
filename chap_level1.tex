...
The effect of this change on the performance of the \L1 trigger is benchmarked in Section (\ref{subsec:l1jetseedpu}). The two run periods that are used in this chapter to compare trigger performance before and after the introduction of a central jet seed threshold are known as Run 2012B and Run 2012C respectively. They reflect the period prior and after the extended shutdown period of the \ac{lhc} when this change was implemented. 
 ...
 Additionally the \L1 \ac{GCT} performance is also measured as a function of different pile-up conditions which occurred during the run period in Section (\ref{subsec:l1jetpu}).
...
\begin{itemize}
	...
	\item energy sums from the calorimeters supplied by the \ac{RCT} are used in performing jet clustering (described in the following section). The clustered jets are then sub-divided into categories depending on their pseudo-rapidity and the result of $\tau$ identiÞcation, being classified as either central, forward, or tau ($\tau$). After being sorted by decreasing energy, the four highest of each category are passed to the \ac{GT} for use in trigger decisions,
...
To form the jet candidates the \ac{GCT} utilises a pre-clustering algorithm, which employs 18 jet-finders that operate in parallel over the whole detector. Each jet-finder spans an area of 11 calorimeter regions in $\eta$ (half the detector) and two in $\phi$ (40$\degree$).  
