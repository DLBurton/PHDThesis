...
Using the statistical framework outlined in the previous chapter, results are compared to a \ac{SM}-only hypothesis (Section (\ref{sec:smhypothesis})) and interpreted using various \ac{SMS} models (Section (\ref{sec:resultsms})). 
...
Different $n_{jet}$ and $n_{b}^{reco}$ bins are used in the interpretation of different \ac{SMS} models. The choice of the categories used are made to increase sensitivity to that particular type of final state signature. The production and decay modes of the \ac{SMS} models under consideration are summarised in Table \ref{tab:susyresults}, with limit plots of the experimental reach in these models shown in Figure \ref{fig:smslimitplots}.

The models \texttt{T1} and \texttt{T2} are used to characterise the pair production of gluinos and first or second generation squarks, respectively, with parameters for the sparticle mass as well as on the \ac{LSP} mass. The simplified models \texttt{T2bb}, \texttt{T1tttt}, and \texttt{T1bbbb} describe various production and decay mechanisms in the context of third-generation squarks. 
...
Experimental uncertainties on the \ac{SM} background predictions (10 $-$ 30\%, Section (\ref{subsec:determinesystematics})), the luminosity measurement (4.4\%), and the total acceptance times efficiency of the selection for the considered signal model (12 $-$18\%, Section (\ref{sec:smsmodels})) are included in the calculation of the limit. 
...
The estimates on mass limits shown in Table  \ref{tab:susyresults}, are determined conservatively from the observed exclusion based on the theoretical production cross section, minus 1$\sigma$ uncertainty. The most stringent mass limits on pair-produced sparticles are obtained at low \ac{LSP} masses, while the limits typically weaken for compressed spectra points close to the diagonal. In particular, for all of the considered \ac{SMS} models, there is an \ac{LSP} mass beyond which no limit can be set, which can observed from the figures referenced in the table. 
