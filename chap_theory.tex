...
Within this chapter, a brief introduction and background information to the \ac{SM} is given. The \ac{SM}'s success as a rigorously tested and widely accepted theory is discussed as are its deficiencies which lead to the argument that this theory is not a complete description of our universe. The motivations for new physics at the \TeV scale and in particular Supersymmetric theories are outlined within Section (\ref{sec:susytheory}). The chapter concludes with how an experimental signature of such theories can be produced and observed at the \ac{lhc} in Section (\ref{sec:susysearches}).

\section{The Standard Model}

\label{sec:thesm}

The \ac{SM} is the name given to the relativistic Quantum Field Theory, where particles are represented as excitations of fields, which describe the interactions and properties of all the known elementary particles \cite{Glashow:1961tr}\cite{Salam:1968rm}\cite{Hooft1971167}. It is a renormalisable field theory which contains three symmetries: $SU(3)$ for colour charge; SU(2) for weak isospin and; U(1) relating to weak hyper charge, which requires its Lagrangian \lsm to be invariant under local gauge transformation. 
...
Gluons                   & g                 & 1 & 0			 &   0 \\ 
Higgs Boson         & H                & 0 & 0			 &   125.3 $\pm 0.5$\\ 
\end{tabular*}
\end{center}
\caption[The fundamental particles of the \ac{SM}, with spin, charge and mass displayed.]{The fundamental particles of the \ac{SM}, with spin, charge and mass displayed. Latest mass measurements taken from \cite{pdg2012}, with the Higgs mass best fit value given by \cite{Chatrchyan:2012ufa}. }
\label{tab:sm_particles}

\end{table}

Fermions are separated into quarks and leptons of which only quarks interact with the strong nuclear force. Quarks unlike leptons are not seen as free particles in nature, but rather exist only within baryons, which are composed of three quarks with an overall integer charge, and quark-anti-quark pairs called mesons. Both leptons and quarks are grouped into three generations which have similar properties, but with ascending mass in each subsequent generation. 
...
Invariance of the Lagrangian is then achieved by replacing $\partial_{\mu}$ with $D_{\mu}$:
...
The $SU(2)_{L}$ group is the special unitary group of $2 \times 2$ matrices, $U$, satisfying $UU^{\dagger} = I$ and det(U) = 1. It may be written in the form $U = e^{-i\omega_{i}T_{i}}$, with the generators of the group written as $T_{i} = \frac{1}{2}\tau_{i}$ where $\tau_{i}$, $i \in$ 1,2,3 are the $2 \times 2$ Pauli matrices:
...
where the left-handed fermions form isospin doublets $\chi_{L}$ and the right handed fermions are isosinglets $\fermfield_{R}$. For the first generation of leptons and quarks this represents
...
Local gauge invariance within $\mathcal{L}_{EWK}$ is once again imposed by modifying the covariant derivative
...
where the mixing angle, $\theta_{w} = \tan ^{-1} \frac{g^{'}}{g}$, relates the coupling constants of the neutral weak and electromagnetic interactions. 
...
\phi^{+} \\
\phi^{0} \end{array} \right)  \qquad \text{with} \qquad
\begin{array}{c} 
...
The ground state of the \phi field is defined to be consistent with the $V(\phi)$ minimum, and is chosen to ensure the maintenance of an unbroken electromagnetic symmetry. This in turn preserves a zero photon mass \cite{weinberghiggs} and leads to
...
The three massless Goldstone boson fields are removed by setting $\theta(x)$ to zero and substituting $\phi(x)$ back into kinetic term of $\mathcal{L}_{Higgs}$. From Equation (\ref{higgslagrangian}), this leads to mass terms for the $W^{\pm}$ and Z bosons. This is given by, 
...
\label{subsec:ckmmatrix}

Yukawa couplings between the Higgs field and fermions introduce mass terms for the fermions. With three generations of quarks within the \ac{SM}, the Yukawa couplings induce 3$\times$3 mass matrices with off-diagonal matrix elements to the quark mass terms. The consequence is a mixing of quark generations or ``flavour mixing''. This mixing matrix in known as the CKM matrix \cite{PhysRevLett.10.531}\cite{Kobayashi:1973fv}, which is named after after Nicola Cabibbo, Makoto Kobayashi and Toshihide Maskawa.

The CKM matrix is a 3$\times$3 unitary matrix and is completely specified by three mixing angles and a complex phase. Its connects the weak eigenstates of down-type quarks (d$^{'}$, s$^{'}$, b$^{'}$) and their corresponding mass eigenstates (d, s, b) through,
...
where the various $\lvert V_{\text{ij}}\rvert^{2}$ represent the probability that the quark of $j$ flavour decays into a quark of $i$ flavour via the weak interaction. 

The CKM matrix thus allows for transition between mass eigenstates for up and down type quarks in electroweak couplings between quarks and the W$^{\pm}$ bosons. Conversely, due to the unitary property of the matrix, it does not affect the coupling between quarks and the \Zboson boson thus accounting for the suppression of such decays in the SM at tree-level. The measured value of the parameter $V_{\text{tb}}$ in the CKM matrix is important to understanding the decay of the top quark and why the top quark forms an integral part to the supersymmetric searches described in this thesis.

\subsection{The Top Quark}

\label{subsec:topquark}

The existence of the top quark was first predicted by the discovery of the bottom quark in 1977 \cite{Harari1975265}. However due to the large mass of the top quark, it was not experimentally confirmed until 1995 by the D$0$ \cite{D0exp} and the \ac{CDF} \cite{cdfexp} experiments at the Tevatron collider \cite{tevatron}, making it the most recently discovered quark.

The production of top quarks at particle colliders is mainly dominated by the pair production of top quarks (\ttbar). Due to its very large mass of 173.5 \GeV \cite{pdg2012}, it has a very short lifetime of the order of $5\times10^{-25}$s \cite{tquark_lifetime}. This lifetime is about 20 times smaller than the typical time scale for strong interactions. Therefore the top quark decays weakly before it is able to form hadrons. 

The main decay mechanism of top quarks is via the process

\begin{equation}
t \rightarrow W^{+}b \qquad \text{and} \qquad \bar{t}\rightarrow W^{-}\bar{b},
\end{equation}

for which the branching ratio is close to 1. Therefore the decay to $t \rightarrow W^{}s$ and $t \rightarrow W^{}d$ is heavily suppressed. The CKM matrix element $\lvert V_{\text{tb}}\rvert$, quantifying the probability of a top quark to decay to a W boson and a b-quark was measured to be 0.91$\pm$0.13 and 1.07$\pm$0.12  by the \ac{CDF} and the D$0$ experiments respectively \cite{Group:2009qk}. 

A \ttbar event would thus primarily decay into two W bosons and two b-quarks. The W bosons in turn can decay hadronically into a quark anti-quark pair, $W \rightarrow q\bar{q}$, with a branching ratio of $\sim \frac{2}{3}$ or leptonically into a lepton and a neutrino, $W \rightarrow l\nu$, with a branching ratio of $\sim\frac{1}{3}$.

Due to its very high mass and well defined properties, the top quark plays an important role in the consistency tests of the \ac{SM} and as a calibration tool within particle detectors. Within the context of this thesis the top quark, via the technique of b-tagging (see Section (\ref{subsec:cmsobjects-btagging})) is used to aid in the identification of possible signatures of Natural \ac{SUSY} models, which is introduced in the following sections.
...
This can be demonstrated by considering the one loop quantum correction to the Higgs mass with a fermion f, shown in Figure \ref{fig:higgscorrection} with mass $m_{f}$. The Higgs field couples to f with a term in the Lagrangian $-\lambda_{f}h\bar{f}f$, yielding a correction of the form \cite{Martin:1997ns},
 ...
 \begin{equation}
 \delta m^{2}_{h} = -\frac{\lvert\lambda_{f}\rvert^{2}}{8\pi^{2}}\Lambda^{2} + É.. ,
 \end{equation}
...
\subsection{R-Parity and Dark Matter}

\label{subsec:rparity}

Supersymmetric theories can also present a solution to the dark matter problem. These theories contain a stable \acf{LSP}, which match the criteria of a \acf{WIMP} required by cosmological observation when R-parity is conserved.  
...
A \ac{LSP} will not interact in a detector at a particle collider, leaving behind a missing energy, $\met$, signature. The assumption of R-parity and its consequences are used to determine the physical motivation and search strategies for \ac{SUSY} at the \ac{lhc}. 


 \subsection{Natural SUSY}

\label{subsec:natsusy}

One of the main arguments for Supersymmetry at the \ac{EWK} scale is motivated by solving the hierarchy problem and natural electroweak symmetry breaking is one of the leading explanations for why we might expect to discover superpartners at the \ac{lhc}  \cite{Hisano:2000wy}\cite{Papucci:2011wy}. As introduced in the previous section, the introduction of superpartners can lead to the cancellation of divergent terms to the Higgs mass negating the need for the fine-tuning of parameters. 

The largest contribution to the quadratic divergence in the Higgs mass parameter comes from a loop of top quarks via the Yukawa coupling. Cancellation of these divergences can be achieved in supersymmetric theories by requiring a light right-handed top squark (stop), $\widetilde{t}_{R}$, and left-handed double $SU(2)_{L}$ doublet containing top and bottom squarks, $(\frac{\widetilde{t}}{\widetilde{b}})_{L}$ \cite{Allanach:2012vj}. 

This bottom-up approach allows for very heavy first and second generation squarks beyond the reach of the \ac{lhc}, whilst still providing a solution to the hierarchy problem where these quadratic corrections from coupling to top quarks are cancelled out by the light stops as shown in Figure \ref{fig:stopcancellation}.

\begin{figure}[ht]
\footnotesize
\centering
\includegraphics[width = 0.65\linewidth]{plots/stopcancellation}
\caption[Higgs mass corrections due to the \ac{SM} top quark (left) are cancelled out by a light supersymmetric stop particle (right).]{Higgs mass corrections due to the \ac{SM} top quark (left) are cancelled out by a light supersymmetric stop particle (right).}
\label{fig:stopcancellation}
\end{figure}

This naturalness argument implies that these third generation squarks should have a mass range smaller than 1 \TeV to be compatible with the now discovered Higgs boson \cite{Kribs:2013lua}. Discovery of sparticle masses in this range lie well within the reach of the \LHC during it first years of running, making the hunt for a natural \ac{SUSY} signature an attractive option to early supersymmetric searches. How such a signature would look is described in the following section. 
...
Each \ac{SMS} model is defined in a two-dimensional plane of these two free parameters, the mass of the parent squark/gluino pair and the \ac{LSP}. It would be extremely computationally intensive to simulate the decays of every single parent-\ac{LSP} mass point for use in the interpretation of the physics reach of an analysis. Therefore, mass points within this two-dimensional plane are generated in simulation with a granularity of 25 \GeV. This value is chosen as trade off between allowing for limited computing resources to generate sufficient coverage of the \ac{SMS} parameter space whilst maintaining sufficient sensitivity to any changes in the final state kinematics across the \ac{SMS} plane.

An example of such a plane in which the results of this thesis are interpreted is shown in Figure \ref{fig:smsexampleplane}. The results for each \ac{SMS} model are then presented on these planes as upper limits on production cross sections, as a function of model mass parameters.

\begin{figure}[ht]
\footnotesize
\centering
\includegraphics[width = 0.65\linewidth]{plots/sms_exmple}
\caption[An example of the two-dimensional plane through which \ac{SMS} results are interpreted within this thesis. ]{An example of the two-dimensional plane through which \ac{SMS} results are interpreted within this thesis. Each pixel represents a parent gluino/squark and \ac{LSP} mass point which are separated in 25 \GeV steps. The colour within this plot represents the observed acceptance of these simulated signal events to pass the event selection of a given analysis. A higher acceptance indicates a higher sensitivity to the final state signature of that \ac{SMS} mass point.}
\label{fig:smsexampleplane}
\end{figure}
...
\end{minipage}
\caption[Production and decay modes for the various \ac{SMS} models interpreted within this thesis.]{Production and decay modes for the various \ac{SMS} models interpreted within this thesis.}
