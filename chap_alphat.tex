...
\begin{equation}
L_{had} =   \Pi_{i}\text{Pois}(n^{i}\rvert b^{i} + s^{i}),
\end{equation}

where $b^{i}$ represents the expected \ac{SM} background ($EWK_{i} + QCD_{i}$) and $s^{i}$ the expected number of signal events from the different \ac{SMS} models interpreted. Pois refers to the Poission distribution of these values and is defined as :
