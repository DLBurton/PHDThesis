...
Using the $\alphat$ search as a base, a simple templated fit is employed to estimate the \ac{SM} background in higher b-tag multiplicities (3-4) from a region of a low number of reconstructed b-jets (0-2). As a proof- of-concept, the procedure after being shown to close in simulation, is applied to the SM enriched \mupjets control sample of the \alphat all-hadronic search detailed in Chapter \ref{chap:SUSYsearches}. Results are presented using the \ac{CSV} tagger for the ``Loose'', ``Medium'' and ``Tight'' working points.


\section{Concept}
\label{sec:templateconcept}

As detailed in the previous chapter, the \alphat analysis is a search for \ac{SUSY} particles in all-hadronic final states, utilising the kinematic variable \alphat to suppress QCD to a negligible level. The dominant \ac{SM} backgrounds within the signal regions are \ttbar + jets, W + jets and \zinv + jets. These process are characterised by typically having zero or two b-quarks per event. The first step in this approach is to categorise the templates to be fitted to the low $n_{b}$ multiplicity in terms of underlying b-quark content :

\begin{itemize}
\item[Z0:] W + jets, \zinv, $Z \rightarrow \mu\mu$ , WW, ZZ, WZ
...
The generation of these templates is dependant upon the same four ingredients listed at the beginning of Section (\ref{subsec:backgroundestimation}), with the tagging probabilities of a jet being a function of the jet \pt, the pseudo-rapidity $\rvert\eta\lvert$, and the jet-flavour. This can be observed in Figure \ref{}, where the b-tagging / c-tagging / mis-tagging efficiency for the three working points of the \ac{CSV} tagger is shown as a function of jet \pt.




Using the formula method to determine yields from simulation with higher statistical precision as explained in Section (\ref{subsec:formulamethod}), 

These templates are fit in the low 
