...
Similarly the tagging efficiencies measured in data \cite{btag8tev}, $\epsilon_{Data}(\pt,\eta,f)$, are further related to $\epsilon_{\texttt{FastSim}}(\pt,\eta,f)$ by the equation,
...
For each jet, the weight of the event is re-weighted according to wether the jet fires the b-tagger. In the instance that the jet \emph{is} b-tagged, the event weight will be modified by,
...
When considering \ac{SUSY} signal models within the likelihood, the additional $L_{\text{sig}}$ term is included and therefore when multiple categories are fit simultaneously the total likelihood is then represented by 
